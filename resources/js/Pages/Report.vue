...
const orders = ref([
  { date: 'Mar 28', customer: 'John', product: 'T-shirt', qty: 20, status: 'Pending' },
  { date: 'Mar 27', customer: 'Anna', product: 'Jersey', qty: 15, status: 'Ongoing' },
  { date: 'Mar 26', customer: 'Mark', product: 'Tarpaulin', qty: 5, status: 'Done' },
])

const supplies = ref([
  { name: 'Fabric', category: 'Material', stock: 50, unit: 'meters', status: 'Normal' },
  { name: 'Ink', category: 'Consumable', stock: 10, unit: 'ml', status: 'Low' },
  { name: 'Film', category: 'Material', stock: 30, unit: 'pcs', status: 'Normal' },
])
</script>
                ...
                <td>
                  <span
                    :class="{
                      ...
                      'text-blue-500': order.status === 'Ongoing',
                      'text-green-500': order.status === 'Done',
                    }"
                  >
                    {{ order.status }}
                  </span>
            ...
            <tbody>
              <tr v-for="supply in supplies" :key="supply.name">
                ...
                <td>
                  <span
                    :class="{
                      'text-green-500': supply.status === 'Normal',
                      'text-red-500': supply.status === 'Low',
                    }"
                  >
                    {{ supply.status }}
                  </span>
